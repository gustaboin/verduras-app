...
import { updatePedidoConItems } from '@/lib/pedidos'
...
export default function EditPedidoForm({
  pedidoId,
  clienteIdInicial,
  fechaPedidoInicial,
  ...
  const [form, setForm] = useState({
    ...
    fecha_pedido:     fechaPedidoInicial,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-3xl">
              ...
              <tbody className="divide-y">
                {items.map((item, i) => (
                  <tr key={i} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">{item.es_recortable ? '✓' : '—'}</td>
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          {loading ? 'Guardando...' : `Guardar cambios (${items.length} ítem${items.length !== 1 ? 's' : ''})`}
        </button>
        <button
          type="button"
          onClick={() => router.back()}
